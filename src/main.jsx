...
const faqs = [
  ['What is Sarrun?', 'Sarrun is a non-custodial shielded account protocol for native ETH on Robinhood Chain. It uses private note commitments and zero-knowledge proofs instead of a public account graph.'],
  ...
  return (
    <header className="site-header">
      ...
      <div className="header-end">
        <a className="chain-pill" href={CHAIN.blockExplorerUrls[0]} target="_blank" rel="noreferrer"><i />Robinhood Chain</a>
    ...
    <div className="hero-product">
      ...
      <div className="account-preview">
        <div className="preview-head">
          ...
          <div><small>SHIELDED ACCOUNT</small><b>Robinhood Chain</b></div>
        ...
        <div className="preview-row"><span className="asset-icon robinhood-chain"><img src={`${BASE}robinhood-chain-feather.jpg`} alt="" /></span><div><b>ETH on Robinhood Chain</b><small>NATIVE GAS ASSET · CHAIN 4663</small></div></div>
  ...
  return (
    <section className="hero" id="top">
      ...
      <div className="hero-copy">
        ...
        <p>A non-custodial shielded account protocol for Robinhood Chain. Shield, transfer and exit ETH without publishing your private note graph.</p>
      ...
      <div className="trust-strip">
        ...
        <span><Code2 />Inspectable core</span><span><Zap />Built for Robinhood Chain</span>
      </div>
    </section>
  )
}
...
const pillars = [
  { icon: Fingerprint, n: '01', title: 'Value-bound notes', text: 'Every public ETH deposit is cryptographically bound to one private commitment.', meta: ['BINDS', 'ASSET · VALUE · OWNER'] },
  ...
  return (
    <section className="product-section" id="product">
      <div className="section-intro">
        ...
        <p>Sarrun keeps the note graph private while Robinhood Chain verifies that every state transition follows the protocol rules.</p>
      </div>
...
function FlowSection() {
  const steps = [
    ['Connect', 'Use an EVM wallet on Robinhood Chain for public settlement.'],
    ['Shield', 'Bind public ETH to a private note commitment.'],
    ['Transfer', 'Move value between private notes with a zero-knowledge proof.'],
    ['Exit', 'Prove a valid spend and return ETH to a public recipient.'],
  ]
  return (
...
function SecuritySection() {
  const items = [
    ...
    ['Value bound at entry', 'The contract derives each deposit commitment from the exact ETH value supplied in the transaction.'],
  ...
  return (
    <section className="developer-section" id="developers">
      ...
      <div className="dev-content">
        ...
        <div className="dev-stack"><span>Circom</span><span>Solidity</span><span>Groth16</span><span>JavaScript SDK</span></div>
...
function Footer() {
  return (
    <footer>
      ...
      <div className="footer-bottom"><span>© 2026 Sarrun</span><span>Non-custodial privacy infrastructure for Robinhood Chain.</span><a href={CHAIN.blockExplorerUrls[0]} target="_blank" rel="noreferrer">Mainnet explorer <ArrowUpRight /></a></div>
    </footer>
  )
}
...
function AppShell({ close }) {
  ...
  const refreshBalance = async (address = account) => {
    if (!address || !window.ethereum) return
    const raw = await window.ethereum.request({ method: 'eth_getBalance', params: [address, 'latest'] })
    setBalance((Number(BigInt(raw)) / 1e18).toLocaleString(undefined, { maximumFractionDigits: 5 }))
  }
  ...
  const importRecovery = async (event) => {
    ...
    try {
      ...
      if (record?.version !== 1 || Number(record.chainId) !== 4663) throw new Error('This is not a valid Robinhood Chain Sarrun record.')
  ...
  const connect = async () => {
    if (!window.ethereum) { setError('No browser wallet found. Install an EVM wallet to continue.'); return }
  ...
  const disconnect = async () => {
    ...
    setAccount(''); setBalance(''); setAction(''); setProgress('')
  }

  const handleShield = async (event) => {
    event.preventDefault(); setBusy(true); setError(''); setProgress('signing')
    try {
      const result = await shieldNative({ ethereum: window.ethereum, amount: shieldAmount, onPrepared: record => { setRecovery(record); downloadRecoveryRecord(record) } })
  ...
  const handleTransfer = async (event) => {
    ...
    try {
      ...
      const result = await transferPrivate({
        ethereum: window.ethereum, recovery, amount: transferAmount, paymentAddress,
        onProof: setProgress,
  ...
  const handleExit = async (event) => {
    ...
    try {
      ...
      const result = await exitPrivate({
        ethereum: window.ethereum, recovery, amount: exitAmount, recipientAddress: exitRecipient,
        onProof: setProgress,
        onPrepared: record => { if (record) downloadRecoveryRecord(record) },
      })
  ...
  const scanNotes = async () => {
    ...
    try {
      ...
      const found = await scanReceivedNotes({ ethereum: window.ethereum, receiverRecovery })
  ...
  useEffect(() => {
    ...
    const changed = (accounts) => { setAccount(accounts[0] || ''); if (accounts[0]) refreshBalance(accounts[0]) }
  ...
  return (
    <div className="app-shell">
      <header className="app-header"><Logo dark /><a href={CHAIN.blockExplorerUrls[0]} target="_blank" rel="noreferrer" className="app-network"><i />Robinhood Chain</a><button onClick={close}><X /></button></header>
      ...
      <main className="app-content">
        {activeTab === 'account' && (!account ? (
          <div className="connect-state">
            ...
            <p>Connect an EVM wallet to use Sarrun on Robinhood Chain. Sarrun never asks for your wallet seed phrase.</p>
            <button className="primary-btn green" onClick={connect} disabled={connecting}><Wallet />{connecting ? 'Connecting…' : 'Connect wallet'}</button>
            {error && <div className="wallet-error">{error}</div>}
            <small><Lock />Non-custodial connection</small>
          </div>
        ) : (
          <div className="connected-state">
            <div className="connected-head"><div><span className="eyebrow">CONNECTED ACCOUNT</span><h1>Your Sarrun account.</h1></div><div className="connected-controls"><button className="address-chip" onClick={() => navigator.clipboard?.writeText(account)}>{account.slice(0,6)}…{account.slice(-4)} <Copy /></button><button className="disconnect-wallet" onClick={disconnect}><LogOut />Disconnect</button></div></div>
            <div className="account-grid">
              <section className="main-balance"><span>PUBLIC WALLET BALANCE</span><strong>{balance} ETH</strong><small><i />Robinhood Chain mainnet</small><div><button onClick={() => { setAction('shield'); setError('') }}><Plus />Shield ETH</button><button onClick={() => { setAction('transfer'); setError('') }}><Send />Send privately</button><button onClick={() => { setAction('exit'); setError('') }}><ArrowUpRight />Exit to wallet</button></div></section>
              <section className="shield-status"><ShieldCheck /><span>SHIELDED ACCOUNT</span><h2>{recovery ? 'Spendable note loaded' : 'Private state'}</h2><p>{recovery ? 'A local recovery record is ready to authorize a private transfer or public exit.' : 'Import a note recovery record or shield ETH to create private state.'}</p><b><Lock />Keys remain local</b></section>
            </div>
            {action === 'shield' && <form className="protocol-action" onSubmit={handleShield}><div><span>SHIELD ETH</span><h3>Move value into your private account.</h3><p>The deposit amount is public. The resulting spending secret is generated locally.</p></div><label><span>AMOUNT</span><div><input type="number" min="0" step="0.000001" placeholder="0.00" value={shieldAmount} onChange={event => setShieldAmount(event.target.value)} required /><b>ETH</b></div></label><button className="primary-btn green" disabled={busy}>{busy ? progressLabel : 'Shield assets'} <ArrowRight /></button><button type="button" className="action-close" onClick={() => setAction('')}><X /></button></form>}
            {action === 'transfer' && <form className="protocol-action two-fields" onSubmit={handleTransfer}><div><span>PRIVATE TRANSFER</span><h3>Send to a Sarrun address.</h3><p>The recipient’s encrypted note is published with the proof. A change recovery file is downloaded before signing.</p></div><label><span>AMOUNT</span><div><input type="number" min="0" step="0.000001" placeholder="0.00" value={transferAmount} onChange={event => setTransferAmount(event.target.value)} required /><b>ETH</b></div></label><label><span>SARRUN ADDRESS</span><div><input type="text" placeholder="sarrun:rh:1:…" value={paymentAddress} onChange={event => setPaymentAddress(event.target.value.trim())} required /></div></label><button className="primary-btn green" disabled={busy}>{busy ? progressLabel : 'Generate proof'} <ArrowRight /></button><button type="button" className="action-close" onClick={() => setAction('')}><X /></button></form>}
            {action === 'exit' && <form className="protocol-action two-fields" onSubmit={handleExit}><div><span>PUBLIC EXIT</span><h3>Exit ETH to a public wallet.</h3><p>The exit amount and recipient become public. Any private change remains protected by a new note.</p></div><label><span>AMOUNT</span><div><input type="number" min="0" step="0.000001" placeholder="0.00" value={exitAmount} onChange={event => setExitAmount(event.target.value)} required /><b>ETH</b></div></label><label><span>RECIPIENT</span><div><input type="text" placeholder="0x…" value={exitRecipient} onChange={event => setExitRecipient(event.target.value.trim())} required /></div></label><button className="primary-btn green" disabled={busy}>{busy ? progressLabel : 'Generate proof'} <ArrowRight /></button><button type="button" className="action-close" onClick={() => setAction('')}><X /></button></form>}
