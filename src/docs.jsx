...
import './styles.css'
import './docs.css'
import architectureSource from '../programs/murven/ARCHITECTURE.md?raw'
...
const X_URL = 'https://x.com/murvendotfun'
const BASE = import.meta.env.BASE_URL
function Logo(){return <a className="docs-logo" href={BASE}><span><img src={`${BASE}murven-logo-transparent.png`} alt="" /></span><b>murven</b><i>/docs</i></a>}
...
const files=[
  {type:'folder',path:'programs/murven',description:'Solana PDA, vault, nullifier and verifier boundary'},
  {type:'file',path:'programs/murven/ARCHITECTURE.md',description:'Native Solana program and release design',language:'Markdown',source:architectureSource},
...
function RepoBrowser(){
  ...
  return <div className="repo-browser">
    <div className="repo-bar"><div><Code2/><b>murven / protocol</b></div><Status>SOLANA NATIVE</Status></div>
...
<section className="docs-hero" id="overview"><span className="docs-kicker"><Sparkles/>MURVEN ON SOLANA</span><h1>Architecture,<br/><em>down to the account.</em></h1><p>Murven is a persistent shielded-account protocol designed for Solana. SOL enters a value-bound commitment, moves through zero-knowledge JoinSplits and exits only after the program accepts a known root, fresh nullifiers and a valid conservation proof.</p><div className="docs-callout"><Lock/><p><b>Non-custodial by construction.</b> Wallet Standard signs public settlement instructions; spending and viewing secrets remain local to the user's recovery records.</p></div></section>
...
<section className="docs-section" id="architecture"><Head n="02" label="ARCHITECTURE" title={<>Public settlement.<br/>Private state.</>} copy="Solana verifies state transitions without receiving the private note graph."/><div className="architecture-map"><MapCard tag="PUBLIC" icon={Network} title="Solana" text="Program instructions, PDAs, commitments, nullifiers and public SOL exits."/><ArrowRight className="map-arrow"/><MapCard core tag="SHIELDED" icon={Fingerprint} title="Murven state" text="Private note values, owners, Merkle paths and zero-knowledge transitions."/><ArrowRight className="map-arrow"/><MapCard tag="USER" icon={KeyRound} title="Local keys" text="One-time spending and viewing keys with portable recovery records."/></div><div className="lifecycle">{lifecycle.map(([n,t,p])=><article key={n}><span>{n}</span><h3>{t}</h3><p>{p}</p></article>)}</div></section>
...
<section className="docs-end"><p>Follow protocol releases, security notes and deployment records.</p><div><a className="primary-btn green" href={X_URL} target="_blank" rel="noreferrer">Follow Murven on X <ArrowUpRight/></a><a className="primary-btn docs-outline" href={`${BASE}?app=1`}>Launch Murven <ArrowRight/></a></div><small>Always verify the Solana cluster, program ID, PDA set and artifact hashes before signing.</small></section>
</main></div></div>}
